...
class WeightTracker:
    ...
    def _get_graph(self, members: list[str]):
        ...
        for member in df.columns:
            if member in members:
                ...
                y_axis = [datum for datum in member_data if not math.isnan(datum)]
                x_axis = [datetime.strptime(axis_data[i], '%Y-%m-%d').date() for i, datum in enumerate(member_data) if not math.isnan(datum)]
                colour = self._stringToColour(member)
                plt.plot(x_axis, y_axis, c=colour, linewidth=2, markersize=0, alpha=1)
        ...
        plt.legend(name_list, loc='upper left', bbox_to_anchor=(0.95, 1), frameon=False, prop=fontP)
    ...
    async def __record_weight(self, message: Message):
        ...
        msg_date = datetime.date(message.created_at.astimezone(timezone('Australia/Melbourne')))
        ...
        try:
            ...
            if date[0:4].isnumeric():
                ...
                update_date = datetime(int(date_split[0]), int(date_split[1]), int(date_split[2])).date()
            else:
                date_split = date.split(date[2])
                print(date_split)
                update_date = datetime(int(date_split[2]), int(date_split[1]), int(date_split[0])).date()
            print(update_date)
    ...
    async def __show_graphs(self, message: Message):
        members = []
        if message.mentions:
    ...
    async def handle_message(self, message: Message):
        ...
        if message.content.startswith('graph'):
            await self.__show_graphs(message)
